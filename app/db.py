...
def save_resume_to_firestore_and_session(user_id, generated_resume, resume_id):
    ...
    resume_ref = db.collection('users').document(user_id).collection('resumes').document(str(resume_id))
    resume_ref.set({
...
def set_user_resumes_in_firestore(user_id, user_resumes):
    ...
    for resume_id, resume_content in user_resumes.items():
        user_resume_ref.document(resume_id).set({
...
def get_next_resume_id(user_id):
    ...
    query = resumes_ref.order_by('resume_id', direction=firestore.Query.DESCENDING).limit(1)
...
def get_user_resumes_from_firestore(user_id, page=1, per_page=10):
    ...
    resume_query = resume_ref.order_by('timestamp', direction=firestore.Query.DESCENDING).limit(per_page).offset(start_index).stream()
...
def get_last_10_user_resumes(user_id):
    user_resumes = {}
    resume_ref = db.collection('users').document(user_id).collection('resumes').order_by('timestamp', direction=firestore.Query.DESCENDING).limit(10).stream()
    for doc in resume_ref:
        resume_id = doc.id
        resume_data = doc.to_dict()
        user_resumes[resume_id] = resume_data['content']
